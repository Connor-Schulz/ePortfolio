...
//==============================================================================
  ...
  private class NextListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (updateCycleLocation == portfolioFunctions.getInvestListSize()-1) {nextButton.setEnabled(false);}
      if (updateCycleLocation > 0) {prevButton.setEnabled(true);}
    }
  }
  ...
  private class PrevListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      // Disable/enable next prev buttons if they reach the end of the list
      if (updateCycleLocation == 0) {prevButton.setEnabled(false);}
      if (updateCycleLocation < portfolioFunctions.getInvestListSize()) {nextButton.setEnabled(true);}
...
//==============================================================================
  ...
  private class BuyInvestmentListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      // If no errors are found, run the desired function and update the text with the return
      if (errorMessage.equals("")) {
        if (name.equals("")) {name = " ";}
        messageBox.setText(portfolioFunctions.buy(type,symbol,name,Integer.parseInt(quantity),Double.parseDouble(price)));
      }
    }
  }
  ...
  private class UpdateInvestmentListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      // If no errors are found, run the desired function and update the text with the return
      if (errorMessage.equals("")) {
        messageBox.setText(portfolioFunctions.update(portfolioFunctions.getInvestment(updateCycleLocation),Double.parseDouble(price)));
      }
  ...
  private class SearchInvestmentListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (portfolioFunctions.validateDouble(lowPrice,true) == true && portfolioFunctions.validateDouble(highPrice,true) == true) {
      ...
      if (lowPrice.equals("")) {lowPrice = "-1";}
      if (highPrice.equals("")) {highPrice = "-1";}

      // If no errors are found, run the desired function and update the text with the return
      if (errorMessage.equals("")) {
        messageBox.setText(portfolioFunctions.search(symbol,nameKeys,Double.parseDouble(lowPrice),Double.parseDouble(highPrice)));
      }
    }
  }
...
//==============================================================================
  ...
  private void showUpdateMenu() {
    ...
    // Previous Button
    prevButton = formatButton(new JButton("Prev"),100,150,investmentsExist && portfolioFunctions.getInvestListSize() > 1 && updateCycleLocation != 0);
    PrevListener prevListener = new PrevListener();
    prevButton.addActionListener(prevListener);

    // Next Button
    nextButton = formatButton(new JButton("Next"),100,150,investmentsExist && portfolioFunctions.getInvestListSize() > 1);
...
//==============================================================================
  ...
  private void modifyScrollMessageBox(String message, int fontSize, int height) {
    ...
    messageScrollBars = new JScrollPane (messageBox, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
